...
const { sendSlackNotification } = require("./notifySlack"); // Import Slack function
...
const scanWebsite = async (url) => {
  ...
  while (currentUrl && !visitedUrls.has(currentUrl)) {
    ...
    for (const form of forms) {
      const { validationLog: formValidationLog, formHasValidation } =
        await scanFormFields(form, page);
    ...
    validationLog.forEach((fields, formIndex) => {
      ...
      fields.forEach((log, index) => {
        ...
        console.log(
          `   - Validation Messages: ${
            Array.isArray(log.validationMessages)
              ? log.validationMessages.join("; ")
              : log.validationMessages
          }`
        );
      });
    });
    ...
    // Send Slack notification after scanning
    //     const slackMessage = `🚀 Scan Complete for ${url}
    //     - Fraud Risk Score: ${fraudScore} ${fraudScore > 100 ? '(Critical Risk 🚨)' : fraudScore > 50 ? '(High Risk ⚠️)' : '(Low Risk ✅)'}
    //     - Forms Found: ${forms.length}
    //     - External Validation: ${externalValidationURLs.size > 0 ? [...externalValidationURLs].join('\n') : 'None'}
    //     - Affiliate Pixels Detected: ${detectedPixels.length > 0 ? detectedPixels.join(', ') : 'None'}
    //     `;
...
try {
  ...
  console.error(
    "⚠️ Failed to load config.json. Ensure the file exists and is formatted correctly."
  );
  process.exit(1);
}
